...
from Ui_windowProgressBar import Ui_WindowProgress
...
T_CIR = 2           # Concepto del importe registrado
T_NPOLIZA = 3       # Numero de poliza
T_ENTASEG = 4       # Entidad aseguradora
T_IMP = 5           # Importe ingreso - egreso


class ProgressBar(Ui_WindowProgress):
    def __init__(self):
        self.dial = QtGui.QDialog()
        Ui_WindowProgress.setupUi(self, self.dial)
...
class MyForm(Ui_MainWindow):
    ...
    def save_as_docx(self):
        ...
        if self.tabla.rowCount() > 0:
            ...
            for e in regs:
                progress.progressBar.setValue((i*100.0)/tam)
                fechaRegistro = e[1]['fechaRegistro']
                concepto = e[1]['concepto']
                ciaId = e[1]['ciaId']
                moneda = monedas[e[1]['importeTipo']]['Signo']
                importe = moneda + ' ' + e[1]['importe']
                # Fill table
                # rows[0].text = str(i)
                rows[T_FREG-1].text = fechaRegistro
                rows[T_CIR-1].text = concepto
                rows[T_NPOLIZA-1].text = '\n'.join(e[1]['polizas'])
                rows[T_ENTASEG-1].text = ca[ciaId]['Denominacion']
                rows[T_IMP-1].text = unicode(importe)
